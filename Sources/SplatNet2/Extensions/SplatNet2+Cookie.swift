...
extension SplatNet2 {
    ...
    internal func refreshToken(sessionToken: String) async throws -> UserInfo {
        ...
        // スプラトゥーントークン
        let splatoonToken: GameServiceToken.Response = try await getGameServiceToken(accessToken: accessToken, version: version)
        // スプラトゥーンアクセストークン
        let splatoonAccessToken: GameWebToken.Response = try await getGameWebToken(accessToken: splatoonToken, version: version)
        // イカスミセッション
    ...
    /// スプラトゥーントークン取得
    internal func getGameServiceToken(accessToken: AccessToken.Response, version: String) async throws -> GameServiceToken.Response {
    ...
    /// スプラトゥーンアクセストークン取得
    internal func getGameWebToken(accessToken: GameServiceToken.Response, version: String) async throws -> GameWebToken.Response {
        let imink: Imink.Response = try await getIminkToken(accessToken: accessToken)
        let request: GameWebToken = GameWebToken(imink: imink, accessToken: accessToken, version: version, authType: .SP2)
        return try await authorize(request)
    }
    ...
    /// イカスミセッション取得
    internal func getIksmSession(accessToken: GameWebToken.Response) async throws -> IksmSession.Response {
