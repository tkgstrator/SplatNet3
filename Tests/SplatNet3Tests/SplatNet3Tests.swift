...
//        print(result)
    ...
    func testCookie() async throws {
        do {
            ...
            let splatoonToken: GameServiceToken.Response = try await session.getGameServiceToken(accessToken: accessToken, imink: iminkNSO, version: version)
            ...
            let splatoonAccessToken: GameWebToken.Response = try await session.getGameWebToken(accessToken: splatoonToken, imink: iminkAPP, version: version)
